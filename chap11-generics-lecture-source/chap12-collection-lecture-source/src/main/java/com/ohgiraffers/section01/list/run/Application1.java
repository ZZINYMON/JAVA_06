...
//        stringList.add(123); // 스트링타입으로해서 정수x
        ...
        Collections.sort(stringList);   // clollection은 인터페이스, Collections는 클래스, sort쓰면서 분류가능. 오름차순한 것.
...
//        System.out.println("stringList = " + stringList);
        ...
        * 인덱스로 관리하는 컬렉션이 아닌 경우에는 반복문을 사용해서 요소에 하나씩 접근할 수 없기 떄문에
        * 인덱스를 사용하지 않고도 반복문을 사용하기 위한 목록을 만들어주는 역할이라고 보면 된다.
        * hashNext() : 다음 요소를 가지고 있는 경우 true, 더 이상 요소가 없는 경우 false 를 반환
        * next() : 다음 요소를 반환
        *  */
